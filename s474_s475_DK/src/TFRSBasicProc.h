...
class TFRSBasicProc : public TGo4EventProcessor {
public:
  TFRSBasicProc() ;
  TFRSBasicProc(const char* name);
  virtual ~TFRSBasicProc();
protected:
  ...
  TH1I* MakeH1ISeries(const char* foldername, Int_t seriesnumber, Int_t crate, Int_t number, Bool_t remove);
    TH1I* MakeH1ISeriesV1290raw(const char* foldername, Int_t seriesnumber, Int_t crate, Int_t number, Bool_t remove);
...
private:           
  typedef TNamed* (TFRSBasicProc::*getfunc)(const Text_t*);
  ...
  ClassDef(TFRSBasicProc,1)
};

#endif //TFRSBASICPROCESSOR_H
