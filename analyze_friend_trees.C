...
void analyze_friend_trees(TString file1){

  // open file with FRS data (upacked using go4)
  TFile *out_file = new TFile(file1.Data());
  // take tree from file with FRS data                                                   
  TTree *tree = (TTree*)out_file->Get("AnalysisxTree");
  ...
  tree->SetBranchAddress("FRSAnlEvent.id_AoQ",&AoQ);
  tree->SetBranchAddress("FRSAnlEvent.id_z2",&z2);
